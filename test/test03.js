...
describe('fit template engine tests with options changed', function() {
	it('assign', function(done) {
		...
		s.forEach(function(s1, idx) {
			t(s1, {async: 'yes', pre: 'no', l_del: '<{', r_del: '}>'})({name:'fillano'}, function(result) {
				assert(expect, result);
				if(s.length === idx+1) done();
			});
		})
	});
	it('assign with complex expression', function(done) {
		var expect = '<div>lastname, firstname</div>';
		var s = '<div><{=$name[1]}>, <{=$name[0]}></div>';
		var data = {name: ['firstname', 'lastname']};
		t(s, {async: 'yes', pre: 'no', l_del: '<{', r_del: '}>'})(data, function(result) {
	...
	it('for', function(done) {
		...
		s.forEach(function(s1, idx) {
			t(s1, {async: 'yes', pre: 'no', l_del: '<{', r_del: '}>'})({names: [{name:'name1'},{name:'name2'},{name:'name3'}]}, function(result) {
				assert.equal(expect, result);
				if(s.length === idx+1) done();
			});
		});
	...
	it('if expression is false with else tag', function(done) {
	...
	it('with or without pre compiled template should have the same result', function(done) {
		...
		t(s, {async: 'yes', pre: 'yes', l_del: '<{', r_del: '}>'})(data, function(result) {
			assert.equal(expect, result);
			done();
		});
	});
	it('for scope variable _global in single for loop', function(done) {
	...
	it('for scope variable _global in double for loop', function(done) {
	...
	it('for reusing template with different data', function(done) {
		var expect1 = '<div>name1</div>';
		var expect2 = '<div>name2</div>';
		var s = '<div><{=$name}></div>';
		var data1 = {name:'name1'};
		var data2 = {name:'name2'};
		t(s, {async: 'yes', pre: 'no', l_del: '<{', r_del: '}>'})(data1, function(result) {
			assert.equal(expect1, result);
		});
		t(s, {async: 'yes', pre: 'no', l_del: '<{', r_del: '}>'})(data2, function(result) {
			assert.equal(expect2, result);
			done();
		});
	});
	it('for plain old string array', function(done) {
	...
	it('for plain old number array', function(done) {
	...
	it('for loop without `as` expression', function(done) {
	...
	it('for plain old boolean array', function(done) {
	...
	it('for plain old null array', function(done) {
	...
	it('for plain old undefined array', function(done) {
	...
	it('for complex for within if', function(done) {
		...
		t(s1, {async: 'yes', pre: 'no', l_del: '<{', r_del: '}>'})(data, function(result) {
			assert.equal(expect, result);
		});
		t(s2, {async: 'yes', pre: 'no', l_del: '<{', r_del: '}>'})(data, function(result) {
			assert.equal(expect, result);
			done()
		});
	});
    it('for unknown token', function(done) {
        ...
        assert.equal(expect, t(s, {async: 'yes', pre: 'no', l_del: '<{', r_del: '}>'})(data));
    });
    it('for unknown token in if/else', function(done) {
        var expect = '<div><{unknown(while)}></div>';
